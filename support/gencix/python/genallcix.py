...
# builtins to start
module_names = [
    "*", "time", "sys", "gc", "operator", "cPickle", "marshal", "imp", "new",
...
module_names = list(set(module_names).union(
    sys.builtin_module_names).difference(ignore_modules))
...
def gen_modules():
    ...
    for path in paths:
        ...
        for (dirpath, dirnames, filenames) in os.walk(path, True):
            ...
            if ("win32" not in dirpath and "site-packages" in dirpath):
                continue
            for filename in filenames:
                extension = os.path.splitext(filename)[1]
                if extension in EXTENSIONS:
                    fullpath = os.path.join(dirpath, filename)
                    if fullpath in _already_processed_filepaths:
                    ...
                    modpath = shortpath[:-len(
                        extension)].replace(os.path.sep, '.')
...
if sys.version_info >= (2, 6):
    import operator
    module_names.sort(key=operator.methodcaller(
        'lower'))  # canonicalize the sort order
else:
...
def fixup_cgi_module(cgi_elem):
    ...
    if el:
        el.set(
            "signature", "parse(fp=None, environ=os.environ, keep_blank_values=0, strict_parsing=0)")
