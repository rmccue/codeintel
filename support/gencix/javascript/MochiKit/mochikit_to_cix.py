            ...
            if sp[0] in ("return", "returns"):
                ...
                if pos < len(sp):
                    ...
                    if returnType in known_javascript_types:
                        ...
                        # print "  Found return type: %r, from doc: %r" %
                        ...
                        for name in className.split("."):
                            if name.lower() == returnType:
                                ...
                                # % (returnType, docstring)
                                return className
                        # print "  Unrecognized return type: %r, from doc: %r"
                        # % (returnType, docstring)
...
def processSection(cix_blob, cix_module, section_type, div_tag):
    ...
    for p_tag in div_tag.findAll("p", recursive=False):
        ref_tags = p_tag.findAll(attrs={
                                 "class": "mochidef reference"}, recursive=False)
        if not ref_tags:
            continue
        if len(ref_tags) != 1:
            raise "Invalid number of reference tags found: %d" % (
                len(ref_tags))
        ...
        if section_type == "Errors":
            ...
            if len(name_split) == 1:
                ...
                if len(sig_split) > 1:
                    ...
                    cix_element = createCixFunction(cix_class, name)
                    setCixSignature(cix_element, "%s(%s" % (
                        name, sig_split[1]))
                else:
        ...
        if cix_element.get("ilk", None) == "function":
            ...
            if not returnType:
                ...
                returnType = getReturnTypeFromDocString(
                    doc, cix_module.get("name"))
            ...
            if returnType:
                ...
                setCixSignature(cix_element, "%s(%s => %s" % (
                    name, sig_split[1], returnType))
            else:
                setCixSignature(cix_element, "%s(%s" % (name, sig_split[1]))

        # All done, now we need to create an alias/link from main scope
        # First, find that scope, building up a name list on the way
        scope_path = "MochiKit.%s.%s" % (cix_module.get(
            "name"), cix_element.get("name"))
        createCixVariable(cix_blob, cix_element.get(
            "name"), vartype=scope_path)
...
def parseWithSoup(cix_root, scopename, data):
    ...
    api_references = soup.html.body.findAll(
        attrs={'id': "api-reference"}, limit=1)
    if api_references:
        ref_tag = api_references[0]
        h1_tag = ref_tag.parent
        # print h1_tag
        #<variable name="MochiKit" citdl="Object">
        cix_file = createCixFile(cix_root, "MochiKit/%s.js" % (
            scopename), lang="JavaScript")
        cix_blob = createCixModule(cix_file, scopename + ".js", "JavaScript")
        mochikit_variable = createCixVariable(
            cix_blob, "MochiKit", vartype="Object")
        scope_variable = createCixVariable(
            mochikit_variable, scopename, vartype="Object")
...
def main(cix_filename):
    cix_root = createCixRoot(
        name="MochiKit", description="A lightweight JavaScript library - v%s" % (MOCHIKIT_VERSION))

    # svn checkout of MochiKit trunk
    co_dir = os.path.abspath("mochikit_svn")
    remove_directory(co_dir)
    p = os.popen("svn co http://svn.mochikit.com/mochikit/tags/MochiKit-%s mochikit_svn" %
                 (MOCHIKIT_VERSION))
