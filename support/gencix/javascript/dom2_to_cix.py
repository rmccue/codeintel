...
def generateCIXFromXML(root, filepath):
    ...
    cixmodule = createCixModule(cixfile, os.path.basename(
        filepath), lang="JavaScript")
        ...
        for exceptionnode in exceptionnodes:
            ...
            for componentnode in exceptionnode.findall('./component'):
                ...
                cixelement = createCixVariable(
                    cixclass, componentname, typename)
            ...
            for groupnode in root.findall('.//group'):
                for constantnode in groupnode.findall('./constant'):
                    ...
                    cixelement = createCixVariable(
                        cixclass, constname, vartype=typename, attributes="static")
                    setCixDocFromNodeChildren(
                        cixelement, constantnode, './descr')
        ...
        for interfacenode in interfacenodes:
            ...
            for attrnode in interfacenode.findall('.//attribute'):
                ...
                if typename:
                    typename = standardizeJSType(typename)
                cixelement = createCixVariable(
                    cixclass, attrname, vartype=typename)
            ...
            for methodnode in interfacenode.findall('.//method'):
                ...
                for paramnode in methodnode.findall('./parameters/param'):
                    if paramnode.get("attr") == "in":
                        ...
                        addCixArgument(cixelement, argname, standardizeJSType(
                            paramnode.get("type")))
                ...
                for returnsnode in methodnode.findall('./returns'):
                    addCixReturns(cixelement, standardizeJSType(
                        returnsnode.get("type")))
            ...
            if zipdata:
                ...
                for zfile in zf.filelist:
                    ...
                    if os.path.dirname(zfile.filename) == "definitions":
                        ...
                        possibleFiles[zfile.filename] = StringIO(
                            fixXMLEntities(zf.read(zfile.filename)))
                    elif zfile.filename == "":
                        pass
...
def main():
    ...
    cix_dom2_file = createCixFile(
        cix_dom2, "javascript_dom2", lang="JavaScript")
    ...
    f = file(os.path.join(os.path.dirname(
        __file__), "dom2.cix"), "w").write(get_cix_string(cix_dom2))
